...
Rules live in three layers (§13). Every diagram passes the single verify entrypoint before handoff (§14). Agents and chained skills enter through the front door (§15).
...
Open [`references/style-guide.md`](references/style-guide.md) and check the default tokens. If they're still the shipped defaults (paper `#f5f5f5`, ink `#2d3142`, accent `#eb6c36` atomic-tangerine), first check for chained-skill design artifacts per the detection order in [`references/onboarding.md`](references/onboarding.md) § Chained sources; when one is found, offer to derive the style guide from it before the manual menu. Otherwise **pause and ask the user**:
...
**Fresh diagrams ship as a round of 4 by default** — one file, four complete takes that disagree structurally (composition, emphasis, density, and when honest, a second visual type), flipped by a CSS-only position bar. Load [`references/variants.md`](references/variants.md) for the contract, the skip conditions (single-version asks, pinned redraws/imports, edits, motion-first requests), and the handoff shape. The round runs after the style-guide gate and type routing; announce it in the same pre-draw message ("shipping 4 takes — say 'just one' to skip").

**Offer reveal-on-load motion when — and only when — the content is inherently ordered.** Static is always the default; motion is opt-in and never rescues a diagram (§4, `animation.md`). When the diagram's meaning is sequential or accumulating — any semantic pattern, or the architecture, process, sequence, flowchart, data-flow, timeline, layer-stack, and loop types — fold a one-line offer into that same pre-draw message, e.g. *"Static by default — want it to assemble step-by-step on load (reveal)?"* Stay silent for types where order carries no meaning (Venn, scatter, quadrant, radar, treemap, bar, ER, nested, tree, org chart): dangling motion there invites animation that adds nothing. If the user doesn't answer, ship static. When they opt in, load `animation.md` and follow the `reveal` mode contract there.
...
| Any breach of the seven §6 connector rules | Diagonal slants, labels touching their stroke, masks clipped by a later node, overlapping paths, shared attach points, transit behind a non-endpoint box, terminal approaches under 48px — each is an automatic fail; §6 states them in full |
...
These seven rules are **non-negotiable**. Run the pre-output checklist (§9) to verify before producing any diagram.
...
6. **A label mask must not overlap a node drawn after it.** Rule 2 keeps the label off its own connector; this one keeps it off the boxes. Because nodes are painted after labels, a mask that lands partly inside a node is covered by the node fill and the text renders as a fragment sitting on the node border. Place the label on a segment of the connector that runs through open canvas — for a connector leaving a node's right edge, that means clearing the node's `x + width` before the mask starts. A mask fully *inside* a node is a badge chip and is fine; a mask overlapping a zone container is fine too, since zones are painted first. The geometry check inside `verify.py` (§14) enforces this rule.

7. **Every arrowhead needs a visible terminal approach.** The final straight segment after the last bend must be at least 48px; 64–96px is preferred. A corridor narrower than 48px reads as a floating arrowhead, not a directional path. If the layout cannot provide it, recompose per [quality-hardening.md](references/quality-hardening.md) §3–4 (lanes, snake, convergence stack) rather than patching paths.
...
For faithful architecture maps, diagrams near the budget ceiling, or any first render with crowded cards or connectors, load [quality-hardening.md](references/quality-hardening.md): copy budgets per card kind, rendered clearance minimums, and lane-based recomposition patterns.
...
- [ ] **Every marked connector shows ≥48px of straight approach before its arrowhead, entering the natural destination edge, with the marker family matching the stroke (§6 rule 7)?**
- [ ] **No label mask overlaps a node drawn after it? (Node fill would clip the text — §6 rule 6; the geometry check inside `verify.py` catches this.)**
...
- [ ] If animated, does the complete static/no-JS frame work, does reduced motion hide/disable playback, and is the controller copied verbatim from `assets/template-motion.html`?
- [ ] If animated, does every connector step with its **destination** node (animation.md rule 6)? No arrow may be visible before the box it points into; wrap arrows + labels in `data-motion-item` groups with descriptive aria-labels, keeping them in the arrow paint layer.
- [ ] Did `python3 scripts/verify.py <file>` exit 0, at the highest tier the environment allows? (The one verification entrypoint — §14. It runs self_check, check_slop, geometry, card-lines, profile gates, motion when marked up, and the rendered browser checks at T1, then writes `<name>.verify.json`.)
- [ ] Reviewed the T1 screenshots against [quality-hardening.md](references/quality-hardening.md) §7? At T2/T3, disclosed that rendered geometry was not verified — never claim the browser check passed below T1.
- [ ] Feels crowded, cramped, or mechanically generated despite passing checks? Load [quality-hardening.md](references/quality-hardening.md) — copy budgets, clearance minimums, the 48px terminal approach, layout recomposition.
...
**Variant round** (default for fresh diagrams) — see [variants.md](references/variants.md). Start from `assets/template-variants.html`: four complete takes in one file behind a zero-JS radio switcher (positions, directions, tradeoffs). The kept position is then delivered as a normal single-diagram file. Rounds are always static; motion applies only to the kept take.
...
- **Locked tokens:** every color and `font-family` in the file references a named custom property from one `:root` block (`var(--paper)`, `var(--accent)`, `var(--font-mono)`) mapped from the style guide. No inline hex/rgb/oklch values mid-document — a value that doesn't exist as a token gets lifted into the block, then referenced. One token block is what makes contrast auditable numerically and lets a variant re-skin by swapping the block alone.
...
## 13. The three-layer rule model

Every rule in this skill belongs to exactly one layer. The test: a rule is Layer 1 only if a diagram violating it is objectively worse in any style.

**Layer 1: invariants.** Measurable, style-agnostic, enforced by `scripts/verify.py` (§14). No profile may override them. The list: the seven §6 connector rules, legend placement, arrow-label masks, no vertical writing-mode text, rendered card clearance ≥8px (warning under 12px), card copy budgets (≤3 supporting lines, ≤5 total content lines), marker color matching stroke semantics, the accessible SVG contract, single-file safety, reduced-motion compliance, and the anti-slop punctuation errors. They live in this file and [quality-hardening.md](references/quality-hardening.md); the scripts are their enforcement.

**Layer 2: profile opinions.** The active profile's voice, parameterized and machine-readable: shadow policy, radius range, accent budget, mono policy, dark mode, glow, hierarchy channel, mood axes. They live in the fenced YAML block of each profile file; the shipped default's block is in [style-guide.md](references/style-guide.md) § Layer-2 opinions, and the format is documented in [profiles.md](references/profiles.md) § Layer-2 opinion block. `verify.py` reads the active profile's block and gates on those values instead of hardcoded defaults. The guard semantics: dark palettes and glow are legal only when the profile declares them. The profile may; the drawer may not. That blocks trained defaults without blocking a genuine neon or terminal brand.

**Layer 3: craft guidance.** Bad in any style but not deterministically measurable, so prose enforced by self-audit: deletion-first, vary card widths, don't reproduce a renderer's layout, encode hierarchy through some visible channel, the 4/10 density target and node thresholds (`faithful` is the only exemption). It lives as prose in §1, §4, §7 and the type references, and in the §9 taste gate.

When editing a rule, place it first: a new measurable universal goes to Layer 1 and a script; a new taste call goes to the profile block; everything else is Layer 3 prose.

---

## 14. Verification: one entrypoint, bounded fix loop

Every diagram passes verification before handoff. From the skill directory:

```bash
python3 scripts/verify.py path/to/diagram.html [--profile SLUG_OR_PATH] [--dark] [--screenshots IMG ...]
```

This is the only verification command; the individual check scripts (self_check, check_slop, verify-geometry, verify-motion, verify_browser, profile_gates) run underneath it and are not invoked directly except when debugging one finding.

**Tier probe.** The script probes agent-browser and runs at the highest achievable tier:

- **T1 browser-verified.** agent-browser drives a rendered pass ([verify-browser.md](references/verify-browser.md)): getBBox clearance, page overflow, console errors, card-vs-card overlap, connector transit, label collisions, motion-step visibility. Only T1 may claim "browser check passed".
- **T2 screenshot-assisted.** Automation blocked but user-supplied `--screenshots` serve as the rendered baseline: static checks plus your review of those images.
- **T3 static-only.** Source audits alone. The handoff must state that rendered geometry was not verified.

On probe failure the script prints the provisioning recipe and names the blocking reason; it never self-provisions. Blocking checks fail the run at every tier; slop-vocabulary findings and 8–12px clearance are warnings, listed in the handoff but never blocking.

**Report.** Each run writes `<name>.verify.json` next to the deliverable: tier reached, per-check pass/fail, warnings, fix cycles used, the active profile and its gate values, the component library id (§ [components.md](references/components.md)), screenshot paths, and the output file list. No stamp goes inside the HTML; the handoff message summarizes the report, and the report is the machine-readable surface an orchestrating agent consumes (§15).

**Fix loop, at most 3 cycles.** Draw, verify, fix, re-verify. After three full cycles with surviving blocking findings, stop patching: recompose the layout per [quality-hardening.md](references/quality-hardening.md) §4 (snake, lanes, convergence stack, split) or escalate to the user, with the report listing what survives. Unbounded loops oscillate; shrinking a card to fix clearance creates an overlap.

**Handoff copy.** Before sending the chat message that delivers the diagram, run it through `python3 scripts/check_slop.py --handoff <file|->`. Same tables, plain-text mode; the writing gate covers every word the skill ships, not only the HTML.

---

## 15. Front door: content + design.md → verified diagram

The invocation shape for a human, a chained skill, or an orchestrating agent.

**Content may be prose or structure.** Either a prose brief (a paragraph describing what to diagram; the skill picks type and composes per §3) or a structured outline (nodes, edges, zones the caller already extracted). When both appear, structure wins over prose.

**Profile resolution order,** per [profiles.md](references/profiles.md):

1. An inline design.md supplied at invocation time ingests first ([onboarding.md](references/onboarding.md) § design.md) and its resulting profile activates for this run, beating the marker file. The ingested profile persists in the library, so repeated runs converge. Ingestion is step one of the run, never a separate errand.
2. Else the nearest `.diagram-design` project marker names the profile. The marker is a default, not a lock.
3. Else the shipped default ([style-guide.md](references/style-guide.md)).

The same order parameterizes verification: `verify.py --profile` for an explicit choice, else marker, else default. The skill can also emit the active profile back out as a design.md ([onboarding.md](references/onboarding.md) § Export), so taste round-trips between tools.

**Return contract.** The caller gets three things: the output files, the prose handoff message, and `<name>.verify.json` (§14) carrying tier reached, checks run, pass/fail, fix cycles used, component library id, and the output file list as one field. There is no separate manifest; verify.json is the surface a retry/accept loop composes on. The drawn SVG declares its implementing library via `data-component-library` per [components.md](references/components.md).

---

## Credit

The drawing engine, the type system, and the editorial design language are the work of [Cathryn Lavery](https://github.com/cathrynlavery), from [diagram-design](https://github.com/cathrynlavery/diagram-design) v2.6 (MIT). The live [example gallery](https://cathrynlavery.github.io/diagram-design/) is hers as well. This skill would not exist without it.

Added on top of that base and maintained in [vercel-labs/diagram-design-extended](https://github.com/vercel-labs/diagram-design-extended): the profile system with per-repo marker resolution (§15, [profiles.md](references/profiles.md)), and the 2.7 hardening pass. The 2.7 additions are the three-layer rule model (§13), the single verification entrypoint and the `scripts/` checkers behind it (§14), the seventh connector rule requiring a 48px terminal approach (§6), and the [anti-slop.md](references/anti-slop.md), [components.md](references/components.md), [quality-hardening.md](references/quality-hardening.md), and [variants.md](references/variants.md) references. Attribute those here, not upstream.

Licensed MIT. See [LICENSE](LICENSE); both copyright lines travel with the skill.

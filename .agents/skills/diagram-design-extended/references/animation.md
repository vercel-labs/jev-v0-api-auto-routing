...
5. **Explicit order.** Mark items `data-motion-item data-step="N"` for integer steps 1–8. At most two items enter per step. `data-step` and DOM order are **independent, and DOM order still wins for painting** — the §6 connector rules force arrows before boxes and zone containers before their contents, so a step-3 zone is often drawn before its step-2 neighbor. Assign `data-step` across the existing paint order; never reorder the SVG to make steps sequential, or you re-break z-order and drop label masks behind nodes.
6. **Connectors step with their destination.** An arrow visible before the node it points into is a claim about nothing: the eye follows the line and lands on a ghost. Wrap each connector (and its label) in a `data-motion-item` group assigned to the step of its **destination** node — never its source, or every beat ends with a dangling line. Each beat then closes cleanly: the box appears, and the line explaining how you got there appears with it. The paint position stays in the arrow layer (rule 5); only the step assignment groups them. Connector groups need descriptive `aria-label`s ("Connector: request into policy evaluation"), and the static frame still shows every connector (rule 1). Fan-out arrows that leave one revealed node for several later nodes step with each destination separately when they reveal at different steps. Connector groups do not count against rule 5's two-items-per-step budget; that budget is for semantic nodes.
7. **Stable end.** Completion exposes all items and sets `data-frame="end"`. Replay resets to step 0 first. Pause clears the pending timer and resume continues from the same step.
8. **Scoped state.** Controls operate on their nearest `[data-motion-root]`; IDs, timers, live regions, and step state never cross figure boundaries.
9. **Failure-safe startup.** JavaScript adds `.motion-ready` only after controls are bound and the initial render succeeds. A script error before that point leaves the complete source visible.
...
- Dark diagrams: the hidden resting state is invisible by design, but the *visible* state must still pass the contrast thresholds in [quality-hardening.md](quality-hardening.md) — audit resolved colors numerically, not by eye.
...
Then run the rendered browser gate in [verify-browser.md](verify-browser.md) — its motion checks (no-JS frame, reduced-motion emulation, control operation, static-capture stability) are mandatory for animated output when `agent-browser` is on PATH. Keyboard operation stays a manual check: Tab reaches each native control; Enter/Space operate it; Left/Right/Home/End step without moving focus.

## Anti-patterns

- Unrequested autoplay, autoplay outside the single sanctioned `reveal` run, viewport re-entry, or an endless semantic loop.
- A blank/partial no-JS or reduced-motion frame — most often shipped as an unconditional `opacity: 0` on an animated element instead of a hide gated behind `.motion-ready` (item 8 owns the startup contract).

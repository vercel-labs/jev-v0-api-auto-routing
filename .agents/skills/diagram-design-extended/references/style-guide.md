...
Each profile carries one fenced YAML block stating its editorial opinions as machine-readable values. Scripts read the active profile's block and enforce these values instead of hardcoded defaults. The semantics are "profile may, drawer may not": a profile may change a value, the drawer never overrides one ad hoc. The block below declares the shipped voice. Field meanings and the `inherited: [...]` convention live in [`profiles.md`](profiles.md).

```yaml
shadow_policy: "none"            # shadows are out, borders are in; quoted so YAML keeps the string
radius_range: [0, 10]            # max radius 6-10px or none; shipped tokens 4/6/8
accent_budget: 2                 # accent goes on 1-2 elements max
mono_policy: technical-only      # mono for ports, commands, URLs; names in sans
dark_mode: false                 # dark surfaces only when a profile declares them
glow: false                      # glow only when a profile declares it
hierarchy_channel: [fill, size]  # node-type treatment table plus varied card widths
mood:
  density: 4                     # of 10; target density 4/10
  motion: 2                      # of 10; static by default, reveal is opt-in
  variance: 3                    # of 10; one fixed editorial voice
inherited: []                    # the default defines every field itself
```

...
## Layer-2 opinion block

Each profile body carries exactly one fenced YAML block (```` ```yaml ````) declaring the profile's editorial opinions as machine-readable values. Verification scripts read the active profile's block and enforce those values instead of hardcoded defaults. The semantics are "profile may, drawer may not": a profile may declare a value the default forbids, but the drawer never picks one ad hoc during a generation.

The shipped default profile declares the current skill voice; see the block in `style-guide.md` § Layer-2 opinions.

### Fields

| Field | Type | Meaning |
|---|---|---|
| `shadow_policy` | `none` \| `tinted` \| `any` | Whether elements may carry drop shadows. The default voice is `none`: shadows out, borders in. |
| `radius_range` | `[min, max]` in px | Legal corner radius bounds for boxes. `[0, 0]` means sharp corners only. The default caps at 10px. |
| `accent_budget` | integer | Maximum accent-colored focal elements per diagram. The default is 2. |
| `mono_policy` | `technical-only` \| `any` | Where monospace type is legal. `technical-only` restricts it to ports, commands, URLs, and field types; names stay in the sans family. |
| `dark_mode` | boolean | Whether dark surfaces are part of this skin. Dark palettes are legal only when the profile declares `true`. |
| `glow` | boolean | Whether glow effects are part of this skin. Legal only when the profile declares `true`. |
| `hierarchy_channel` | list of `shape` \| `weight` \| `fill` \| `size` | The visual channels this profile uses to encode node hierarchy. At least one is required; identical treatment on every node is never legal. |
| `mood` | mapping of axis to integer 1-10 | Coarse taste dials: `density`, `motion`, `variance`. Optional axes; a taste source may supply them, and scripts treat them as advisory. |
| `inherited` | list of field names | See below. |

Unknown extra fields are ignored by scripts but preserved on save. A profile missing a field is treated as inheriting the default value for it; prefer stating that explicitly via `inherited`.

### The `inherited: [...]` convention

When a profile is generated from an under-specified source (a `design.md` ingestion, a partial brand handoff), fields the source did not state fall back to the default profile's values. Every such fallback is recorded by name in the generated block:

```yaml
inherited: [shadow_policy, mono_policy, mood]
```

This keeps the profile auditable: a reader can tell which opinions the brand chose and which arrived from the shipped defaults. The default profile itself declares `inherited: []` because it defines every field. When a later edit sets an inherited field explicitly, remove that name from the list.

The producing flow for design.md sources lives in [`onboarding.md`](onboarding.md) § design.md: it maps the file's tokens, typography, and motion stance into a named profile in this format, fills this block, records fallbacks in `inherited`, and activates the result for the run via a marker-first direct read. It asks the user only on contradiction within the source, never on under-specification. The inverse export verb (same section) serializes the active profile back out as a design.md; fields named in `inherited` collapse out of the exported file and re-inherit on re-ingestion.
